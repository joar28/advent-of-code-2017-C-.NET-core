...
class AdventOfCode_02
{
    static void Main(string[] args)
    {
        var testSheet = new List<int[]>{
            new int[]{5,1,9,5},
            new int[]{7,5,3},
            new int[]{2,4,6,8}
            };
        Console.WriteLine(MakeCheckSum(testSheet) == 18);
        var sheetString = 
...
66	87	176	107	2791	109	21	92	3016	2239	1708	3175	3210	2842	446	484";
        var spreadSheet = MakeTestList(sheetString);
        Console.WriteLine($"Check sum = {MakeCheckSum(spreadSheet)}");

        var testSheet2 = new List<int[]>{
            new int[]{5,9,2,8},
            new int[]{9,4,7,3},
            new int[]{3,8,6,5}
            };
        Console.WriteLine($"EvenlyDevisable test = {EvenlyDevisable(testSheet2)}");
        Console.WriteLine($"EvenlyDevisable = {EvenlyDevisable(spreadSheet)}");
        
    }

    static long MakeCheckSum(List<int[]> SpreadSheet)
    {
        long checkSum = 0;
    ...
    static long EvenlyDevisable(List<int[]> SpreadSheet)
    {
        long checkSum = 0;

        foreach(var row in SpreadSheet)
        {
            foreach(var numA in row)
            {
                foreach(var numB in row)
                {
                    if ((numA/numB) == 1)
                        continue;
                    if ((float)((float)numA / (float)numB) % 1 == 0)
                    {
                            checkSum += numA / numB;
                    }
                }
    ...
    static List<int[]> MakeTestList(string sheetString)
    {
        var spreadSheet = new List<int[]>();
        foreach(var rowString in sheetString.Split('\r'))
        {
            var rowStringArray = rowString.Split('\t');
            var rowArray = new int[rowStringArray.Count()];
            int i = 0;
            foreach (var numStr in rowStringArray)
            {
                rowArray[i++] = Int32.Parse(numStr);
            }
            spreadSheet.Add(rowArray);
        }
        return spreadSheet;
    }
}
